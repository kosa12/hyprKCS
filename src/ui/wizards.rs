...
pub fn get_conflict_groups(model: &gio::ListStore) -> Vec<Vec<KeybindObject>> {
    ...
    let mut groups: Vec<Vec<KeybindObject>> = map.into_values()
        .filter(|g| g.len() > 1)
        .collect();
        
    groups.sort_by(|a, b| {
        let line_a = a.first().map(|o| o.property::<u64>("line-number")).unwrap_or(0);
        let line_b = b.first().map(|o| o.property::<u64>("line-number")).unwrap_or(0);
        line_a.cmp(&line_b)
    });
...
pub fn create_conflict_wizard(
    ...
    // Wrap index or clamp
    let actual_index = if group_index >= groups.len() { 0 } else { group_index };
    ...
    let title = gtk::Label::builder()
        .label(&format!("Conflict {} of {}: {} {}", actual_index + 1, groups.len(), mods, key_char))
        .css_classes(["title-1"])
        .build();
    let subtitle = gtk::Label::builder()
        .label(&format!("{} conflicting definitions found. Select an action for each.", group.len()))
    ...
    for obj in group {
        ...
        let action_label = gtk::Label::builder()
            .label(&if args.is_empty() { dispatcher.clone() } else { format!("{} {}", dispatcher, args) })
            .halign(gtk::Align::Start)
            .css_classes(["heading"])
            .build();
            
        let file_label = gtk::Label::builder()
            .label(&format!("{}:{}", std::path::Path::new(&file_path).file_name().unwrap_or_default().to_string_lossy(), line_num + 1))
            .halign(gtk::Align::Start)
        ...
        delete_btn.connect_clicked(move |_| {
            if let Err(e) = parser::delete_keybind(file_path_buf.clone(), line_num as usize) {
                ...
                refresh_wizard(&stack_c, &model_c, &toast_overlay_c, &wizard_container_c, actual_index);
            }
        });
        ...
        edit_btn.connect_clicked(move |_| {
             if let Some(edit_page_container) = stack_c.child_by_name("edit").and_downcast::<gtk::Box>() {
    ...
    skip_btn.connect_clicked(move |_| {
         refresh_wizard(&stack_c, &model_c, &toast_overlay_c, &wizard_container_c, actual_index + 1);
    });
